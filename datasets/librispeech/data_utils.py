...
def extract_fbank_features(
    ...
    if output_path is not None:
        sf.write(output_path.as_posix(), np.ravel(features), sample_rate)
...
def get_zip_manifest(
        zip_path: Path, feature_root, zip_root: Optional[Path] = None, is_audio=True
):
    ...
    for i in tqdm(info):
        ...
        paths[utt_id] = str(Path.joinpath(feature_root, i.filename))  # f"{zip_path.as_posix()}:{offset}:{file_size}"
        with open(_zip_path, "rb") as f:
            ...
            if is_audio:
                # print('Frames:', sf.info(Path.joinpath(feature_root, i.filename)).frames)
                lengths[utt_id] = sf.info(Path.joinpath(feature_root, i.filename)).frames
            else:
